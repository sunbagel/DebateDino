    ...
    return (
        <div className='flex flex-row flex-wrap gap-4 justify-center'>
            {memberFields.map((f, memberIdx) => {
                return (
                    <Card key={f.id}>
                        <CardHeader className="flex flex-row justify-between min-w-14 items-center">
                        ...
                        {tournament?.form.memberQuestions.map((memberQuestion, memberQuestionIdx) => {
                            ...
                            return (
                                <CardContent key={memberQuestionIdx}>
                                    <FormField
                                        control={form.control}
            ...
            <Button disabled={!(tournament && form.getValues(`teams.${teamIdx}.members`).length < tournament.maxTeams)} onClick={() => memberAppend({
                memberResponses: []
            })} type="button" className="w-full mt-5">
                {tournament && form.getValues(`teams.${teamIdx}.members`).length < tournament.maxTeams && (
                    <p>Add Member</p>
                )}
                {!(tournament && form.getValues(`teams.${teamIdx}.members`).length < tournament.maxTeams) && (
                    <p>You have reached the maximum number of members.</p>
                )}
            </Button>
        </div>
